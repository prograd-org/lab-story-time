...
let remaining = totalNoOfSweets - sweetsConsumedByKaren - (sweetsConsumedInNMeters * metersToTravel);
if(remaining>0){
    return remaining/2;
}
else{
...
for(i=0; i<strategies.length-1; i++){
    bigStrategy = `${bigStrategy}${strategies[i]} `;
}
bigStrategy = `${bigStrategy}${strategies[i]}`;
return bigStrategy.length;
}